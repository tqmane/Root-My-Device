...
#define UMH_CAP_LAST_CAP 40
#define UMH_CAP_HIGH_MASK 0x000001ffU

static int umh_post_exec_contract(void) {
  uid_t ruid = (uid_t)-1;
  uid_t euid = (uid_t)-1;
  uid_t suid = (uid_t)-1;
  gid_t rgid = (gid_t)-1;
  ...
  if (getresuid(&ruid, &euid, &suid) != 0 ||
      ...
      syscall(SYS_setfsuid, -1) != 0 || syscall(SYS_setfsgid, -1) != 0 ||
      getgroups(0, NULL) != 0 ||
      syscall(SYS_capget, &header, caps) != 0 ||
      caps[0].inheritable != 0 || caps[1].inheritable != 0 ||
      caps[0].permitted != UINT32_MAX ||
      caps[0].effective != UINT32_MAX ||
      caps[1].permitted != UMH_CAP_HIGH_MASK ||
      caps[1].effective != UMH_CAP_HIGH_MASK ||
  ...
  for (int cap = 0; cap <= UMH_CAP_LAST_CAP; cap++) {
